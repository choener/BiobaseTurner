...
eInteriorLoop
  :: ( Index c, VG.Vector vc c, VG.Vector ve e, Semiring e, Show (vc c), Ord e )
  => Stack ve c e
  -> IntLoop ve c e
  -> vc c -> vc c
  -> e
{-# Inline eInteriorLoop #-}
eInteriorLoop stack@Stack{..} intloop@IntLoop{..} ls rs
  -- Canonical stack 2x2
  | lls==2, rrs==2 = eStack stack lo0 ro0 ri0 li0 -- note outer, inner pair, both 5'--3'
  -- Stack with 1nt slippage
  | lls==2&&rrs==3 || lls==3&&rrs==2 = eStack stack lo0 ro0 ri0 li0 ⊗ _bulgeL!(Z:.1)
  -- 1x1 interior loop
  | lls==3, rrs==3 = eIntLoop1x1 intloop lo0 ro0 ri0 li0 lo1 ro1
  -- 2x1 interior loops
  | lls==3, rrs==4 = eIntLoop1x2 intloop lo0 ro0 ri0 li0 lo1 ri1 ri2
  | lls==4, rrs==3 = eIntLoop2x1 intloop lo0 ro0 ri0 li0 ro1 li2 li1
  -- 2x2 interior loop
  | lls==4, rrs==4 = eIntLoop2x2 intloop lo0 ro0 ri0 li0 lo1 lo2 ri2 ri1
  -- 2x3 interior loops
  | min lls rrs==4, max lls rrs==5 = _intLoop2x3!(Z:.lo0:.ro0:.lo1:.ro1)
                                   ⊗ _intLoop2x3!(Z:.ri0:.li0:.ri1:.li1)
                                   ⊗ _intLoopL!(Z:.5) ⊗ _intLoopNinio
  -- bulge to the right
  |  (lls==2 && rrs>4 && rrs<32)
  || (lls>4 && lls<32 && rrs==2) = _bulgeAU!(Z:.lo0:.ro0)
                                 ⊗ _bulgeAU!(Z:.ri0:.li0)
                                 ⊗ _bulgeL!(Z:.max (lls-2) (rrs-2))
  -- 1xn loop to the right
  |  (lls==3 && rrs>4 && rrs<32)
  || (rrs==3 && lls>4 && lls<32) = _intLoop1xn!(Z:.lo0:.ro0:.lo1:.ro1)
                                 ⊗ _intLoop1xn!(Z:.ri0:.li0:.ri1:.li1)
                                 ⊗ _intLoopL!(Z:.rrs-2+1)
                                 ⊗ min _intLoopMaxNinio ((rrs-2) `nTimes` _intLoopNinio)
  -- generic interior loops
  | lls>4, rrs>4, lls+rrs>9, lls+rrs<35 = _intLoopMM!(Z:.lo0:.ro0:.lo1:.ro1)
  ...
  | otherwise = trace (printf "eInteriorLoop too large: ls:%s rs:%s\n" (show ls) (show rs)) zero
  where
    lls = VG.length ls
    rrs = VG.length rs
    ...
    -- of the right string.
    lo0 = ls VG.! 0;       li0 = ls VG.! (lls-1)
    lo1 = ls VG.! 1;       li1 = ls VG.! (lls-2)
    lo2 = ls VG.! 2;       li2 = ls VG.! (lls-3)
